...
class NatureCNN(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        self.scale = nn.Parameter(th.zeros(1))
        self.relu = nn.ReLU()
        ...
        #self.fc_out = nn.Linear(64*4*4, 10)
        ...
        # with th.no_grad():
        #     n_flatten = self.forward(th.as_tensor(observation_space.sample()[None]).float()).shape[1]
 
        #self.linear = nn.Sequential(nn.Linear(n_flatten, features_dim), nn.ReLU())
    def use_attention(self, x):
        bs, c, h, w = x.shape #4, 32, 20, 20
        x_att = x.reshape(bs, c, h*w).transpose(1, 2) # 4, 400, 42
...
class NatureCNN_backup(BaseFeaturesExtractor):
    """
    CNN from DQN Nature paper:
        Mnih, Volodymyr, et al.
        "Human-level control through deep reinforcement learning."
